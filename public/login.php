...
<body class="bg-ghost_white-600 min-h-screen">
  <div class="grid md:grid-cols-4 h-screen">
    <div class="hidden md:block md:col-span-3 relative bg-magenta_bloom-500">
    ...
    <div class="col-span-1 flex items-center justify-center bg-white shadow md:rounded-l-xl">
      <div class="w-full max-w-sm p-6">
        <div class="text-2xl font-semibold text-magenta_bloom-600 mb-4">Entrar</div>
        <form method="post" action="login.php" class="space-y-4">
          <?php if (!empty($error)) { echo '<div class="text-magenta_bloom-600 text-sm">' . htmlspecialchars($error) . '</div>'; } ?>
          <div>
          ...
          <button class="bg-magenta_bloom-600 text-white w-full py-2 rounded">Entrar</button>
        </form>
