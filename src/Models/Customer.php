...
class Customer
{
    public static function all(): array
    {
        ...
        return $pdo->query('SELECT * FROM customers ORDER BY id DESC')->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public static function create(string $name, string $cpfCnpj, ?string $email, ?string $phone): ?int
    {
        ...
        $pdo = Database::getConnection();
        $exists = $pdo->prepare('SELECT COUNT(*) FROM customers WHERE cpf_cnpj = ?');
        $exists->execute([$doc]);
        if ($exists->fetchColumn() > 0) return null;
        $stmt = $pdo->prepare('INSERT INTO customers (name, cpf_cnpj, email, phone) VALUES (?, ?, ?, ?)');
    ...
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('SELECT * FROM customers WHERE LOWER(name) = LOWER(?) LIMIT 1');
    ...
    public static function createPlaceholder(string $name): int
    {
        ...
        $stmt = $pdo->prepare('INSERT INTO customers (name, cpf_cnpj, email, phone) VALUES (?, ?, NULL, NULL)');
